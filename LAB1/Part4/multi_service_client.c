...
int main(int argc, char **argv) {
  ...
  int listenfd_tcp, listenfd_udp, connfd_tcp, connfd_udp, tcpPort, udpPort, clientlen;
  ...
  listenfd_tcp = open_listenfd_tcp(tcpPort);
  listenfd_udp = open_listenfd_udp(udpPort);
  
  while(1){
    
    
    clientlen = sizeof(clientaddr); 
    connfd_tcp = accept(listenfd_tcp, (struct sockaddr *)&clientaddr, &clientlen);
